...
export const Label = ({
  ...
  const [xUnit, yUnit] = GraphUtils.scaleGridUnit(range, scale)
...
export const Labels = () => {
  ...
  const [axisCenterX, axisCenterY] = GraphUtils.scaleAxisCenter(
    axisCenter,
    range,
    scale
  )
  ...
  for (
    ...
    if (x < scale[0] - 10 || !endArrow) {
      let l = GraphUtils.unscalePoint(x, range, scale)[0]
      // arrowhead is 10px
      labels.push(
        <Label
          key={`x-label-${x}`}
  ...
  for (let y = axisCenterY - (yShowZero ? 0 : yStep); y >= 0; y -= yStep) {
    if (y > 10 || !endArrow) {
      let l = GraphUtils.unscalePoint(y, range, scale)[1]
      // arrowhead is 10px
      labels.push(
        <Label
          key={`y-label-${y}`}
